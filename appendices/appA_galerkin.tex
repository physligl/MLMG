...
\begin{keybox}
\textbf{目的：}把\chapref{ch:two-grid}的式~\eqref{eq:galerkin}展开成可逐行检查的一维计算，验证本书 nodal FD 代表例中的标准 linear prolongation/full-weighting 下，Galerkin coarse operator 确实回到标准 $2h$ Poisson 离散。cell-centered transfer 的几何位置与权重不同，不由本附录这一套 nodal 公式代表。建议先复习\chapref{ch:two-grid}的 $R$、$P$ 定义。
\end{keybox}
...
\end{equation}
也就是说，在这个经典组合下，Galerkin 粗算子恰好等于直接在 $2h$ 网格上重新离散的 Poisson 算子。

这段推导非常重要，因为它把“Galerkin 是抽象矩阵乘法”和“几何粗网格还是同一个 PDE”连接起来。复杂系数、复杂边界或 AMG 中，这种完美一致性通常不再成立。
