...
\section*{推导与证明题}
\begin{enumerate}
\item 对纯 Neumann Poisson，从连续方程、守恒型有限差分、有限体积和离散矩阵四个层面分别说明常数零空间与兼容条件。指出 FD 与 FV 在何种共享通量假设下给出同一离散兼容条件，以及这一代数结论为何不意味着两种离散构造普遍等价。
...
\section*{编程与上机题}
\begin{enumerate}
\item 从零实现一个最小 2D geometric multigrid：Poisson 显式装配/无矩阵算子应用、加权 Jacobi、限制、延拓、V-cycle。先用\textbf{连续 MMS}验证离散二阶精度，再用固定离散问题/离散 manufactured state 验证 solver 的 mesh-independent convergence；说明为什么不能用同一个测试逻辑混淆这两个结论。
...
\item 任选 AMReX、hypre、PETSc、MueLu、Ginkgo 或 AmgX，与自己的最小 solver 解同一个 case。固定库版本/commit，记录 setup、hierarchy、transfer、smoother、bottom solver 与总时间，并把源码中的 public entry、cycle driver、level state 和 profiler event 映射回本书数学对象。
\end{enumerate}
