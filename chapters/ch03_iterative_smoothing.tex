...
直接法通过有限次代数变换把系统化为更容易求解的形式。一般非奇异矩阵可以写成
\begin{equation}
A=LU,
\end{equation}
随后依次求解下三角和上三角系统。若 $A$ 对称正定，还可以使用 Cholesky 分解
\begin{equation}
A=LL^T.
\end{equation}
...
\toprule
方法类别 & 核心动作 & 是否利用迭代历史 & 本书中的主要角色\\
\midrule
LU / Cholesky & 分解后做三角求解 & 不适用 & 小规模问题、bottom solve、基准对照\\
Jacobi / GS & 重复固定局部修正规则 & 否 & 基本 solver、multigrid smoother\\
CG / GMRES & 在 Krylov 子空间中构造搜索方向 & 是 & 大规模外层迭代求解器\\
Multigrid & 在不同尺度上处理不同误差成分 & 层级信息 & solver 或 preconditioner\\
\bottomrule
\end{tabularx}
...
+\Oh(N^{-4})
\]
推导固定 reduction factor 下 $k=\Oh(N^2)$，并转换成 $k=\Oh(h^{-2})$。
...
\item 对一系列逐步加密网格，测量达到固定 residual reduction 所需的 Jacobi/GS 迭代数，并验证是否接近 $\Oh(h^{-2})$ 的趋势。
