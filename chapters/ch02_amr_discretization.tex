...
如果插值使用依赖当前解的限制器、非线性重构或状态相关权重，则 $w_j=w_j(\bm u)$，上述关系不再定义一个固定矩阵 $A$。此时应把 复合算子 视为非线性映射 $\mathcal A(\bm u)$，或在线性化后讨论其 Jacobian。把这种状态相关的幽灵值填充强行解释成固定稀疏矩阵，会混淆离散模型与求解器对象。
...
最简单的 分片常数传递 可以把这个信息损失写成一个 $2\times2$ 投影矩阵。对一维 $r=2$，令
\begin{equation}
P_0=
...
\end{equation}
$P_0R$ 把任意 细层状态 $(u_{F_-},u_{F_+})^T$ 变成两个相同的均值。它保留平均模态 $(1,1)^T$，却消去细尺度差分模态 $(1,-1)^T$。因此 平均下传的“信息损失”不是抽象说法，而可以直接看成某些 细尺度模态 被 投影 的零空间消去。
...
对新出现的 细层单元，$P_{\mathrm{AMR}}$ 的具体形式取决于离散变量的含义。若未知量是 单元平均值，并且它代表某个守恒密度，则仅要求点值插值精度还不够；新生成的细层单元平均值 还应满足相应的积分守恒关系。对于一个被 $r^d$ 个等体积细层单元覆盖的粗层单元$C$，最基本的约束是
\begin{equation}
\boxed{
