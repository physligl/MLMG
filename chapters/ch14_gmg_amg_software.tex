...
\label{eq:software-complexities}
\end{equation}
后面会看到，PETSc GAMG 甚至直接在 setup 源码中统计并打印 operator complexity。

\section{AMReX MLMG：Cycle Driver 与 Linear Operator 分离}
...
\subsection{\texttt{solve()} 只负责外层求解控制}
...
\subsection{\texttt{oneIter()} 同时处理 AMR hierarchy 与 MG hierarchy}
...
\subsection{\texttt{mgVcycle()} 几乎就是教材公式的直接实现}
...
\subsection{Bottom solver 是显式策略边界}
...
\subsection{Consolidation 改的是 \texttt{DistributionMapping}}
...
\section{hypre BoomerAMG：Setup 是层级制造流水线}
...
\subsection{第一步：从 $A_\ell$ 构造 strength matrix}
...
\subsection{第二步：Coarsening 不是一个算法，而是一组策略分派}
...
\subsection{第三步：Interpolation 同样是策略分派}
...
\subsection{第四步：Coarse operator 是真实的分布式 sparse RAP}
...
\section{hypre 的 V-Cycle：不是递归函数，而是显式状态机}
...
\subsection{下降阶段就是 SpMV + Restriction Matvec}
...
\subsection{上升阶段就是 Prolongation Matvec-Add}
...
\subsection{Smoother 分支比 Cycle 本身复杂得多}
...
\section{PETSc PCMG：把每一层做成可组合对象}
...
\subsection{\texttt{PCMGMCycle\_Private()} 是最清楚的递归 V-Cycle}
...
\subsection{PETSc 的核心不是“实现一个 MG”，而是组合现有对象}
...
\section{PETSc PCGAMG：Hierarchy Factory 建完以后复用 PCMG}
...
\subsection{GAMG 通过函数表替换 coarse-space 算法}
...
\subsection{Coarse-process reduction 直接进入 \texttt{createlevel()}}
...
\section{MueLu、Ginkgo 与 AmgX 应该怎样继续读}
