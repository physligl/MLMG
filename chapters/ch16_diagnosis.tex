...
\item 若数值行为一致，再把总时间定位到细层、中间层、粗层，以及 smooth、transfer、halo、reduction、bottom solve 等具体阶段；
...
迭代数相同但总时间增加 & 执行或数据移动变慢 & 分阶段计时、带宽、等待时间\\
细层快、粗层不再变快 & 固定开销或并行度不足 & 启动/同步开销、有效工作量\\
线程增加后性能变平 & 带宽或 NUMA 限制 & 可持续带宽、远端访问量\\
MPI rank 增加后效率骤降 & 通信延迟/全局归约主导 & halo 等待、Allreduce、每 rank 局部未知量\\
\bottomrule
\end{tabularx}
\end{center}
