...
这三个层次分别对应 bandwidth/cache、occupancy/resource pressure 和 coarse-level parallelism，不能混成一个“GPU utilization”。
...
对 $512^3$ finest grid，这一项本身约为 $3.43$ GiB；还没有算 coefficients、geometry、Jacobi ping-pong、temporary reductions、AMR ghost cells 或 coarse operators。GPU 显存容量因此可能比 FLOP/s 更早成为规模上限。
...
Nsight Systems 适合回答“时间到底花在哪个阶段、有没有 host gap、copy、同步、很多短 kernel、stream 是否真正重叠”；Nsight Compute 则适合进入某个具体 kernel，看 launch configuration、memory workload、occupancy、warp states 和 Roofline\cite{nsightSystems2026,nsightCompute2026}。
